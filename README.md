
# Spotify Display (Music Display Maybe?)

This was a guided project on a little Spotify Display, but My unique twist is going to be Making it work with youtube and eventually sound cloud too. Right now it takes what your currently listening too and displays the name Artist, and duration on the song. 

I made the project, because I've never touched hardware before, and I think things with screena are cool. Everything I'm doing right now is brand new to me, from cad, to coding in C++. I did this project to help me learn the basics for one of my dream projects I want to make.


## The Case

<img width="1762" height="1025" alt="image" src="https://github.com/user-attachments/assets/35d421a7-563e-44b8-956f-90713d526a11" />

<img width="1541" height="898" alt="image" src="https://github.com/user-attachments/assets/bd632c5b-bca6-46c6-9ff2-e72069973536" />

## Wiring Diagram 
<img width="853" height="859" alt="image" src="https://github.com/user-attachments/assets/6a9d6a00-daa0-4c0a-acb3-0d9739d0bc19" />

## BOM!

| Name            | Purpose                                                                                                                   | Quantity | Total Cost (USD) | Link                                                 | Distributor |
|-----------------|---------------------------------------------------------------------------------------------------------------------------|----------|------------------|------------------------------------------------------|-------------|
| Esp32 Usb C     | This is the Brains and what connects all the other things to the pc to actually control stuff and put stuff on the screen | 1        | 4.52             | https://www.aliexpress.us/item/3256805767190776.html | AliExpress  |
| 1.8 TFT Screen  | This is the Display Screen for the entire project                                                                         | 1        | 4.68             | https://www.aliexpress.us/item/3256806984965431.html | AliExpress  |
| Mechanical Keys | Its for the macros, smallest is pack of 10.                                                                               | 1        | 0.99             | https://www.aliexpress.us/item/3256807593325348.html | AliExpress  |
| M4 Inserts      | To Screw the Screen to the Plastic Case                                                                                   | 1        | 2.20             | https://www.aliexpress.us/item/3256808711257006.html | AliExpress  |
| Dupoint Cable   | To connect the keyboard switches and screen to the Ep 32                                                                  | 1        | 2.30             | https://www.aliexpress.us/item/3256803454364801.html | Aliexpress  |
Est Total before shipping and Tax! = $14.69
